...
- 웹 + Chrome Extension: 소스의 0.5.3 버전은 PriceScan 검색 한 번으로 다나와·에누리·쿠팡의 공개 웹 AI 조사와 네이버의 사용자 감시형 AI 조사를 하나의 결과에 합칩니다. 확장 프로그램은 로그인된 네이버 화면의 보이는 텍스트와 공개 상품 링크만 읽고, 서버 AI가 상품·가격을 판독합니다. 쿠키·비밀번호·입력값·스크린샷은 전송하지 않으며 로그인·캡차·보안 확인·판독 불확실 상태에서만 사용자에게 넘깁니다. AI가 연결되지 않으면 기존 고정 파서로 조용히 대체하지 않습니다.
