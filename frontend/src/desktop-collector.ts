...
export type DesktopBridge = {
  version: string;
  start: (payload: { query: string; productId: string; token: string; sources: string[]; sortMode: string; captureMode?: 'manual_scroll' }) => Promise<{ id: string }>;
  list: () => Promise<DesktopJob[]>;
  authorize: (token: string) => Promise<void>;
  logout: () => Promise<void>;
  action: (jobId: string, source: string, action: 'focus' | 'resume' | 'cancel') => Promise<void>;
  showScroll: (jobId: string) => Promise<void>;
  captureAll: (jobId: string) => Promise<{ count: number }>;
...
export const desktopStateLabels: Record<string, string> = {
  ...
  ready_to_capture: '수집 준비 완료',
  needs_sort: '최저가 정렬 필요',
