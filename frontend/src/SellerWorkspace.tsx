...
export default function SellerWorkspace({ token, busy, progress, selectedSources, searchQuota, settingsLabel = "관리자설정", onSearchReserved, onToggleSource, onSettings, onLogout }: {
  ...
  const search = async (title = query, permissionGranted = aiSearchAllowed) => {
    ...
    try {
      ...
      const supervisedBrowser = Boolean(desktop);
      setStatus(supervisedBrowser
        ? `전용 앱 Chromium에서 ${selectedSources.length}개 쇼핑몰 조사를 준비합니다.`
        : "AI가 선택한 쇼핑몰의 공개 가격 정보를 동시에 조사하고 있습니다.");
      const searched = await api<SellerProduct>(token, "/assistant/price-search", {
        ...aiPriceSearchRequest(title, selectedSources, supervisedBrowser),
      });
      onSearchReserved?.();
      acceptProduct(searched, true);
      setView("search");
      if (supervisedBrowser) {
        const mergeRunId = searched.search?.run?.id;
        if (!mergeRunId) throw new Error("쇼핑몰 결과를 합칠 검색 실행을 찾지 못했습니다.");
        await desktop!.start({
          query: title.trim(), productId: searched.id, mergeRunId, token, sources: selectedSources, sortMode: "lowest", captureMode: "ai_supervised",
        });
      }
      const count = searched.search?.items.length || 0;
      setStatus(supervisedBrowser
        ? `${selectedSources.length}개 쇼핑몰을 앱 Chromium에서 조사하고 있습니다. 각 쇼핑몰 상태가 모두 완료될 때 결과가 확정됩니다.`
        : count
  ...
  return <div className={`seller-workspace ${view === "search" ? "is-search-view" : "is-products-view"} ${result?.run ? "has-results" : ""} ${chatOpen ? "chat-open" : ""} ${desktopScrollActive ? "desktop-scroll-active" : ""}`}>
    ...
    {view === "search" ? <section className="seller-search-view seller-page" aria-label="상품 검색과 가격 검토">
      <div className="seller-search-intro"><span>AI PRICE SEARCH</span><h1>찾을 상품만 입력하세요</h1><p>{desktop ? "하나의 앱 Chromium에서 네이버·다나와·에누리·쿠팡을 모두 열고, 화면에 보이는 가격 순위를 한 작업으로 조사합니다." : "웹에서는 공개 검색으로 조사합니다. 로그인 화면까지 정확히 확인하려면 PriceScan Desktop을 사용하세요."}</p></div>
      ...
      {permissionQuery && <section className="seller-search-permission" role="dialog" aria-modal="false" aria-labelledby="seller-search-permission-title">
        <div><span>처음 한 번만 확인</span><strong id="seller-search-permission-title">{desktop ? "4개 쇼핑몰 AI 조사를 시작할까요?" : "공개 웹에서 AI 가격조사를 진행할까요?"}</strong><p>{desktop ? "전용 앱이 선택한 쇼핑몰을 각각 열고 보이는 상품명·가격·링크만 읽습니다. 로그인·캡차·보안 확인이 나오면 해당 쇼핑몰만 멈춰 사용자 확인을 기다리며, 비밀번호·쿠키·결제 정보는 읽거나 전송하지 않습니다." : "입력한 상품명은 설정된 AI에 전송되며, AI가 공개 웹 검색으로 찾은 상품명·가격·판매처·원문 링크만 저장합니다. 브라우저 로그인·쿠키·비밀번호·결제 정보·스크린샷에는 접근하지 않습니다."}</p></div>
      ...
      {!searching && !permissionQuery && <div className="seller-supervised-note"><strong>{desktop ? "4개 쇼핑몰 · 하나의 Chromium 조사" : "검색 한 번으로 AI 자동 조사"}</strong><span>{desktop ? "네 곳을 독립적으로 조사하므로 한 곳의 로그인·보안 확인이 다른 쇼핑몰을 막지 않습니다. 네 곳의 완료·확인 필요·실패 상태를 숨기지 않고 각각 표시합니다." : "선택한 네 쇼핑몰을 서버 AI가 동시에 조사합니다. 공개 웹에서 확인되지 않는 쇼핑몰은 결과에 사유를 표시합니다."}</span></div>}
      ...
      {result?.run && <div className={searching ? "seller-results is-updating" : "seller-results"} aria-busy={searching}>
        ...
        <p className="seller-caption">{result.run.collection_mode === "openai_web_search" ? "AI가 공개 웹 검색의 출처 링크에서 확인한 최저가 후보입니다." : result.run.collection_mode === "hybrid_ai_supervised" ? "일부 쇼핑몰은 앱 Chromium 화면에서, 나머지는 공개 웹 출처에서 확인한 후보입니다." : result.run.collection_mode === "server_managed_browser_agent" ? "선택한 쇼핑몰을 앱 Chromium에서 각각 열어 화면 순위·가격·링크를 판독한 후보입니다." : "이전에 저장된 가격 후보입니다."} 동일 모델·옵션과 배송 조건은 원본 링크에서 검토해 주세요.</p>
